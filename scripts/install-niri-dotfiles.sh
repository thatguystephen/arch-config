#!/usr/bin/env bash
# Install Niri dotfiles and theming
...
DOTFILES_NIRI="${ARCH_CONFIG_DIR}/dotfiles/niri"
DOTFILES_THEMES="${ARCH_CONFIG_DIR}/dotfiles/themes"
USER_CONFIG_DIR="${ACTUAL_HOME}/.config"

echo -e "${BLUE}Installing Niri dotfiles and theming...${NC}"
echo ""

# Check if dotfiles sources exist
if [ ! -d "$DOTFILES_NIRI" ]; then
  echo -e "${RED}Error: Niri dotfiles directory not found: $DOTFILES_NIRI${NC}" >&2
  exit 1
fi
...
ALL_CONFIGS=("${NIRI_CONFIGS[@]}" "${THEME_CONFIGS[@]}")
...
# Install Niri-specific dotfiles
echo -e "${BLUE}Installing Niri-specific dotfiles...${NC}"
echo ""

for config in "${NIRI_CONFIGS[@]}"; do
  SOURCE="${DOTFILES_NIRI}/${config}"
  TARGET="${USER_CONFIG_DIR}/${config}"
...
echo ""
echo -e "${GREEN}✓ Niri dotfiles installed successfully!${NC}"
echo ""
echo -e "${BLUE}Installed configurations:${NC}"
echo "  • Niri compositor settings"
