...
POSITION=$(playerctl --player=chromium,spotify,firefox position)
LENGTH_MICROSECONDS=$(playerctl --player=chromium,spotify,firefox,mpd metadata --format "{{mpris:length}}")

POSITION=$(playerctl --player=chromium,spotify position)
LENGTH_MICROSECONDS=$(playerctl --player=chromium,spotify,firefox,mpd metadata --format "{{mpris:length}}")
